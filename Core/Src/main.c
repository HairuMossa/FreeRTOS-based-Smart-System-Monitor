...
uint32_t SensorTaksBuffer[ 256 ];
osStaticThreadDef_t SensorTaksControlBlock;
const osThreadAttr_t SensorTask_attributes = {
  .name = "SensorTask",
  .cb_mem = &SensorTaksControlBlock,
  .cb_size = sizeof(SensorTaksControlBlock),
  .stack_mem = &SensorTaksBuffer[0],
  .stack_size = sizeof(SensorTaksBuffer),
...
uint8_t CommandQueueBuffer[ 16 * sizeof( CommandMessage_t ) ];
...
uint8_t PrintQueueBuffer[ 16 * sizeof( LogMessage_t ) ];
...
/* Definitions for SystemState */
osMutexId_t SystemStateHandle;
osStaticMutexDef_t SystemStateControlBlock;
const osMutexAttr_t SystemState_attributes = {
  .name = "SystemState",
  .cb_mem = &SystemStateControlBlock,
  .cb_size = sizeof(SystemStateControlBlock),
};
/* USER CODE BEGIN PV */
...
void ManagerFunction(void *argument);
void SensorFunction(void *argument);
...
int main(void)
{
  ...
  osKernelInitialize();
  /* Create the mutex(es) */
  /* creation of SystemState */
  SystemStateHandle = osMutexNew(&SystemState_attributes);
  ...
  CommandQueueHandle = osMessageQueueNew (16, sizeof(CommandMessage_t), &CommandQueue_attributes);

  /* creation of PrintQueue */
  PrintQueueHandle = osMessageQueueNew (16, sizeof(LogMessage_t), &PrintQueue_attributes);
  ...
  ManagerTaskHandle = osThreadNew(ManagerFunction, NULL, &ManagerTask_attributes);

  /* creation of SensorTask */
  SensorTaskHandle = osThreadNew(SensorFunction, NULL, &SensorTask_attributes);

  /* creation of PrintTask */
  PrintTaskHandle = osThreadNew(PrintFunction, NULL, &PrintTask_attributes);
...
/* USER CODE BEGIN Header_ManagerFunction */
/**
* @brief Function implementing the ManagerTask thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_ManagerFunction */
void ManagerFunction(void *argument)
{
  /* USER CODE BEGIN ManagerFunction */
  /* Entry Function */
  ManagerFunction_Entry(argument);
  /* USER CODE END ManagerFunction */
}

/* USER CODE BEGIN Header_SensorFunction */
/**
* @brief Function implementing the SensorTask thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_SensorFunction */
void SensorFunction(void *argument)
{
  ...
  SensorFunction_Entry(argument);
  /* USER CODE END SensorFunction */
}

/* USER CODE BEGIN Header_PrintFunction */
/**
* @brief Function implementing the PrintTask thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_PrintFunction */
void PrintFunction(void *argument)
{
  /* USER CODE BEGIN PrintFunction */
  /* Entry Function */
  PrintFunction_Entry(argument);
